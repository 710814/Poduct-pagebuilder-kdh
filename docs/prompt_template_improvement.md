...
## 8차 작업: 컬러/후면 이미지 일관성 문제 해결 (2026-03-03)

### 프롬프트 내역

8. **사용자 요청**: 이미지 생성 결과물에서 두 가지 문제 보고
   - 컬러별 코디 이미지가 등록한 동일 컬러로 생성되지 않고 조금씩 다른 컬러로 생성됨
   - 후면 이미지를 등록했으나 컬러별 코디 이미지의 후면이 원본 디자인/디테일과 다르게 생성됨
   - 모든 코디컷(후면 포함)은 원본 이미지의 디자인, 디테일, 질감 등을 절대 유지해야 함

### 원인 분석

#### 핵심 버그 1 — 치환되지 않은 프롬프트 전달
- `App.tsx` 430행에서 `generateSectionImage` 호출 시, 컬러 플레이스홀더(`{{COLOR_1}}`)가 치환된 `slotPrompt` 대신 **원본 `slot.prompt`**를 전달
- AI가 `{{COLOR_1}}` 같은 플레이스홀더를 그대로 받아 정확한 컬러를 모르는 상태에서 이미지 생성

#### 구조적 문제 2 — 후면 참조 이미지 미전달
- 후면 슬롯(BACK VIEW) 이미지 생성 시에도 정면 이미지(또는 컬러 이미지의 첫 번째)를 참조로 전달
- 후면 이미지(`role: 'back'`)를 별도로 찾아 전달하는 로직이 없음
- AI는 정면 이미지를 참조로 받아 후면 디자인을 추측하여 그리게 됨

### 수정 내역

#### `App.tsx`
- **다중 이미지 슬롯 처리 블록 (412-436행)**:
  - `slot.prompt` → `slotPrompt` (치환된 프롬프트) 전달로 버그 수정
  - 후면 슬롯(`BACK VIEW`/`back design` 포함) 감지 시 `role: 'back'` 이미지를 참조로 우선 선택
  - 탐색 순서: 해당 컬러옵션의 후면 이미지 → 메인 이미지의 후면 이미지 → 정면 이미지 폴백
- **단일 이미지 프롬프트 블록 (471-489행)**:
  - 동일한 후면 참조 이미지 선택 로직 추가

#### `services/geminiService.ts`
- **`replaceColorPlaceholders()`**: HEX 코드(`hexCode`)가 있으면 컬러명과 함께 포함하여 AI의 정확도 향상
  - 예: `아이보리 (exact color code: #FFFFF0)`
- **`generateSectionImage()`**: REQUIREMENT B에 컬러 정확도 지시 추가
  - "COLOR ACCURACY IS CRITICAL", "DO NOT substitute with a similar color", HEX 매칭 지시
- **`generateSectionImage()`**: 후면 프롬프트 감지 시 참조 이미지가 후면임을 명시하는 서문 추가
  - "BACK VIEW REFERENCE IMAGE PROVIDED", 패턴/질감/스티칭/장식/기장 등 정확 재현 지시

### 빌드 결과

- `npx vite build` 성공 (22.66s)
- 새로운 에러 없음
